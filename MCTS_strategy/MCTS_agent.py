...
"""
To set up how the game works : how to play, when there is a winner
"""
...
def look_for_window(config, player_mark, window):
    return (window.count(player_mark) ==
            4 and window.count(0) == config.inarow - 4)
...
"""
MCTS class which defines the tree and the four phases to build it 
"""
...
class MCTS():
    ...
    def get_ucb(self, node_no):
        if not self.total_parent_node_visits:
            ...
            exploration = math.sqrt(2 *
                                    math.log(self.total_parent_node_visits) /
                                    (self.tree[node_no]['total_node_visits'] +
                                     1))
            ucb_score = value_estimate + self.tunable_constant * exploration
            return ucb_score

    def selection(self):
        """The leaf node with the maximum UCB is selected

        Returns:
            node: leaf node selected
        """  
    ...
    def expansion(self, leaf_node_id):
        """Add new nodes to the current leaf node by taking a random action
              and then take a random or follow any policy to take opponent's action.

        Args:
            leaf_node_id (node): the node selected
    ...
    def simulation(self, child_node_id):
        """Reach the final state of the game starting from the node created by expansion

        Args:
            child_node_id (node): the node created by expansion
    ...
    def backpropagation(self, child_node_id, winner):
        """Update the traversed nodes by backpropagating

        Args:
            child_node_id (node): the node created by expansion
            winner (bool): whether this is a leaf or not
        """  
    ...
    def start_the_game(self):
        """Complete MCTS iteration with all the process running for a certain number of games

        Returns:
            tuple:  the best action
        """        
...
def scores(rewards):
    """Calculate the scores after several games using env.evaluate function between two agents

    Args:
        rewards (int[]): For each game (col), there are the final rewards of the two agents (rows)
    ...
    return {
        'nbr of games': len(rewards),
        'mean_reward': sum(
            r[0] for r in rewards) /
        float(
            len(rewards)),
        'game won': f'{win} on {len(rewards)}'}
