...
class DQN:
    ...
    def choose_action(self, observation):
        ...
        # Get Q(s,.)
        impossible_actions = np.where(state.numpy()[0][0][0] > 0)[0]

        if len(impossible_actions) != cols:
            q_values = self.network(state)
            #print("q_values : ", q_values)
            #print("state : ",state)
        
            for a in impossible_actions:
                q_values[0,a] = float('nan')
    ...
    def learn(self):
        ...
        #print("state1_batch size : ", state1_batch.size())
        #print(q_values)
